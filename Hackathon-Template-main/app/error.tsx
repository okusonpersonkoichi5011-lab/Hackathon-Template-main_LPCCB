...
export default function Error({
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-20 text-center">
      <h1 className="text-xl font-semibold text-slate-900">表示中にエラーが発生しました</h1>
      <p className="mt-3 text-base text-muted-foreground">
        しばらくしてから再度お試しください。
      </p>
      {isDev ? (
        // 開発時のみエラーメッセージを表示（本番ビルドでは出さない）
        <pre className="mx-auto mt-4 max-w-md overflow-auto text-left text-sm text-red-600">
          {error.message}
        </pre>
      ) : error.digest ? (
      ...
      <button
        type="button"
        onClick={() => reset()}
        className="mt-8 rounded-md bg-primary px-4 py-2 text-base font-medium text-primary-foreground hover:opacity-90"
      >
        再読み込み
      </button>
    </div>
